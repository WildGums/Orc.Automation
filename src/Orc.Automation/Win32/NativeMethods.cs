...
public static class ConsoleHelper
{
    ...
    {
        if (NativeMethods1.AttachConsole(NativeMethods1.ATTACH_PARENT_PROCESS) == false
            && Marshal.GetLastWin32Error() == NativeMethods1.ERROR_ACCESS_DENIED)
        {
            // A console was not allocated, so we need to make one.
            if (NativeMethods1.FreeConsole() == false)
            {
